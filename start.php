<?php
	...
	// tasks initialisation function
		function tasks_init() {
			...
			if (isloggedin())
	 	        add_menu(elgg_echo('tasks'), $CONFIG->wwwroot . "mod/tasks/tasks.php");
				
			// Register a page handler, so we can have nice URLs
				register_page_handler('tasks','tasks_page_handler');
				
			// Add our CSS
				elgg_extend_view('css','tasks/css');
			// Add to groups context
				elgg_extend_view('groups/left_column', 'tasks/groupprofile_tasks'); 
			...
			//add submenu options
				if (get_context() == "tasks") {
					...
					if ((isloggedin()) && (page_owner()) && (can_write_to_container(0, page_owner()))) {
						...
						add_submenu_item(elgg_echo('Search based on Skill'),$CONFIG->wwwroot."mod/tasks/tasks.php");
					}
				}
